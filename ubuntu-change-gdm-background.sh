...
answer_release="$(lsb_release -c | cut -f 2)"

if [ "$answer_release" != focal && "$answer_release" != groovy ]; then
    echo 'Sorry, this script only work with Ubuntu 20.04  _OR_  20.10'
    exit 1
fi
...
<gresources>
    ...
    echo 'Usage: sudo ./ubuntu-change-gdm-background /path/to/image.*g'
